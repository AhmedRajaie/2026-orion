...
A strategy is any function: observation -> weights.
The backtester loops over history, asks the strategy for weights each day,
and hands them to the simulator.
"""
...
def run_backtest(
    ...
    weights_by_day = np.zeros(
        (feed.n_days, feed.n_assets),
        dtype=float,
    )
    ...
    result["weights"] = weights_by_day[start:end]
